...
sections.forEach((section, index) => {
  ...
  while ((match2 = rectRegex2.exec(section)) !== null) {
    ...
    hmaPieces.push(`{
      ...
      name: "${originalId}",
      category: "${color === luzColor ? 'base' : 'upper'}",
      x: ${cx - 413},
      y: ${cy - 413},
      rotation: ${rot},
      widthM: ${width / 67},
...
let defs = fs.readFileSync('src/data/hmaDefinitions.ts', 'utf-8');

// Replace HMA_PRESETS entirely!
// Find `export const HMA_PRESETS: HMAPreset[] = [` and replace until end of array.
const startIdx = defs.indexOf('export const HMA_PRESETS: HMAPreset[] = [');
if(startIdx !== -1) {
  // Just rewrite everything from startIdx to the end, since MASTER_VARIANTS is after it, but it's not strictly necessary.
  // Actually, I'll just keep the imports at the top and rewrite the whole file.
}
...
fs.writeFileSync('src/data/hmaDefinitions.ts', newDefsCode);
