...
*(Luz: \`#D7BB11\` | Profundo: \`#8C7907\`)*
  <g id="hma-isotype" data-service="MERCHANDISE">
...
sections.forEach((section, index) => {
  ...
  const titleLine = lines[0].trim(); // HMA ...
  const colorLine = lines[1].trim(); // *(Luz: `#...` | Profundo: `#...`)*
  
  const name = index === 1 ? "HMA MASTER (DIAGONAL)" : titleLine;
  const idStr = name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');

  let luzColor = "#ffffff";
  let profundoColor = "#000000";
  const cMatch = colorLine.match(/Luz:\s*`([^`]+)`\s*\|\s*Profundo:\s*`([^`]+)`/);
  if(cMatch) {
    luzColor = cMatch[1];
    profundoColor = cMatch[2];
  }
  ...
  while ((match2 = rectRegex2.exec(section)) !== null) {
    ...
    // For LogoData (Motion uses this)
    // to1080 scales the coordinates so we should just put raw coords and let to1080 do its job if that's what's currently in data.ts.
    // Wait, if I just pre-calculate to1080 inside data.ts (like TARGET_CENTER + (val - SOURCE_CENTER)), then `x = 540 + cx - 413`
    shapes.push({
      id: originalId,
      length: height, // SVG height is our "length"
      width: width,   // SVG width is our "width" (67)
      x: `to1080(${cx})`,
      y: `to1080(${cy})`,
      rotation: rot,
      color: color
    ...
    // For HMAPiece (used for Animation / header Presets)
    hmaPieces.push({
      id: `piece-${originalId}-${Date.now()}-${Math.floor(Math.random()*1000)}`,
      shapeType: 'P3_rect2x1',
      name: originalId,
      category: color === luzColor ? 'upper' : 'base', // Hack to distinguish them?
      x: cx - 413,
      y: cy - 413,
      rotation: rot,
      widthM: width / 67,
    ...
    hmaPresets.push(`
  {
    ...
    name: "${name}",
    cluster: "Oficial",
    description: "Plantilla oficial ${name}",
    colorLuz: "${luzColor}",
    colorProfundo: "${profundoColor}",
    pieces: [
      ${hmaPieces.map(s => `{
        id: "${s.id}",
        shapeType: "P3_rect2x1",
...
const tsCode = `import { HMAPiece, HMAPreset } from "../types/hma";
...
fs.writeFileSync('src/data/data.ts', tsCode);
