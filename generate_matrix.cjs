...
export const MatrixStudio: React.FC<MatrixStudioProps> = ({ paletteMode, onSendToMotion, showNotification }) => {
  ...
  const handleImportShapes = (imported: MatrixShape[], mode: 'replace' | 'append') => {
    if (mode === 'replace') { setShapes(imported); setSelectedShapeIds(imported.length > 0 ? [imported[0].id] : []); }
    else { setShapes(p => [...p, ...imported]); setSelectedShapeIds(imported.length > 0 ? [imported[imported.length - 1].id] : []); }
  };
  ...
  return (
    <div className="flex flex-col h-full bg-[#1e262c] text-[#eceff1] font-mono text-xs overflow-hidden">
      <div className="flex flex-1 h-full overflow-hidden">
        ...
        <MatrixInspector selectedShapes={shapes.filter(s => selectedShapeIds.includes(s.id))} onUpdate={updates => setShapes(prev => prev.map(s => selectedShapeIds.includes(s.id) ? { ...s, ...updates } : s))} onLayerChange={handleLayerChange} onSnapToGrid={snapAllToGrid} snapMode={snapMode} />
      </div>
