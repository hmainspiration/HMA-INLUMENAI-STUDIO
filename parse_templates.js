...
*(Luz: \`#75C962\` | Profundo: \`#4B893C\`)*
  <g id="hma-isotype" data-service="ILLUSTRATIONS">
...
sections.forEach((section, index) => {
  ...
  const name = index === 0 ? "HMA MASTER (DIAGONAL)" : titleLine; // First one didn't have 00# but I added it for split

  const rects = [];
  const rectRegex = /<rect id="([^"]+)" x="[^"]+" y="[^"]+" width="([^"]+)" height="([^"]+)" rx="[^"]+" ry="[^"]+" fill="([^"]+)" opacity="([^"]+)" transform="translate\(([^ ]+) ([^)]+)\) rotate\(([^)]+)\)" \/>/g;
  
  let match;
  while ((match = rectRegex.exec(section)) !== null) {
    const id = match[1];
    const width = parseFloat(match[2]);
    const height = parseFloat(match[3]);
    const fill = match[4];
    const cx = parseFloat(match[5]); // Actually match[6] because of opacity?
    const cy = parseFloat(match[6]);
    const rot = parseFloat(match[7]);
    
    rects.push({
      id: id + '_' + Date.now() + Math.floor(Math.random()*1000), // unique enough
      width, height, fill, cx, cy, rot // to be processed
    });
  }
  ...
  while ((match2 = rectRegex2.exec(section)) !== null) {
    ...
    actualRects.push({
      id: `shape-${originalId}-${Date.now()}-${Math.floor(Math.random()*1000)}`,
      x: cx - width / 2,
      y: cy - height / 2,
      widthX: width / 67,
      heightX: height / 67,
